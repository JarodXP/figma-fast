...
import { BatchModificationSchema } from '../schemas.js';
...
export function registerBatchTools(server: McpServer): void {
  ...
  server.tool(
    'batch_modify',
    `Modify properties of multiple Figma nodes in a single round-trip. Each modification applies to one node. All changes are batched into a single undo step.
...
{ "modifications": [
      ...
      try {
        ...
        if (response.type === 'result' && response.success) {
          const data = response.data as {
            ...
            failed: number;
            total: number;
            results: Array<{ nodeId: string; name?: string; type?: string; errors: string[]; success: boolean }>;
          };
        ...
        return {
          content: [
            { type: 'text' as const, text: `batch_modify failed: ${err instanceof Error ? err.message : String(err)}` },
          ],
          isError: true,
        };
      }
  ...
  server.tool(
    'batch_get_node_info',
    `Get detailed properties of multiple Figma nodes in a single round-trip. Returns the same data as get_node_info but for multiple nodes at once.
