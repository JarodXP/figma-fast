...
Example: { "nodeId": "123:456" }`,
    ...
    async (params) => {
      ...
      try {
        ...
        return {
          content: [{ type: 'text' as const, text: `Convert failed: ${response.type === 'result' ? response.error : 'Unexpected response'}` }],
          isError: true,
        };
      } catch (err) {
        return {
          content: [{ type: 'text' as const, text: `convert_to_component failed: ${err instanceof Error ? err.message : String(err)}` }],
          isError: true,
        };
      }
    }
...
Example: { "nodeIds": ["123:1", "123:2", "123:3"], "name": "Button" }`,
    ...
    async (params) => {
      ...
      try {
        ...
        if (response.type === 'result' && response.success) {
          const data = response.data as { componentSetId: string; componentSetKey: string; name: string; variantCount: number };
          return {
            content: [{
              type: 'text' as const,
        ...
        return {
          content: [{ type: 'text' as const, text: `Combine failed: ${response.type === 'result' ? response.error : 'Unexpected response'}` }],
          isError: true,
        };
      } catch (err) {
        return {
          content: [{ type: 'text' as const, text: `combine_as_variants failed: ${err instanceof Error ? err.message : String(err)}` }],
          isError: true,
        };
      }
    }
...
{ "componentId": "123:456", "action": "delete", "properties": [{ "name": "Show Icon", "type": "BOOLEAN", "defaultValue": true }] }`,
    ...
    async (params) => {
      ...
      try {
        ...
        if (response.type === 'result' && response.success) {
          const data = response.data as { componentId: string; name: string; action: string; propertiesModified: number };
          return {
            content: [{
              type: 'text' as const,
        ...
        return {
          content: [{ type: 'text' as const, text: `Manage properties failed: ${response.type === 'result' ? response.error : 'Unexpected response'}` }],
          isError: true,
        };
      } catch (err) {
        return {
          content: [{ type: 'text' as const, text: `manage_component_properties failed: ${err instanceof Error ? err.message : String(err)}` }],
          isError: true,
        };
      }
    }
