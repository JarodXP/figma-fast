...
function registerImageTools(server) {
    // ─── set_image_fill ──────────────────────────────────────────
    server.tool('set_image_fill', "Set a node's fill to an image from a URL. The image is downloaded by the server and uploaded to Figma. Supports PNG, JPG, GIF, SVG, and WebP.", {
        nodeId: zod_1.z.string().describe('The Figma node ID to apply the image fill to'),
        imageUrl: zod_1.z.string().url().describe('URL of the image to download and apply'),
        scaleMode: zod_1.z
